...
Current published version: **0.8.0** (see [SDK Changelog](/sdks/changelog)
for wire deltas).

## Stable task facade (recommended)

`BeeOSClient` is a hand-maintained facade for the common
...
`new BeeOSClient()` reads `BEEOS_API_KEY` automatically and uses
`https://openapi.beeos.ai`. Set `BEEOS_API_URL` for another environment, or
pass `{ apiKey, baseUrl }` explicitly when runtime configuration is preferred.
