...
Current published version: **v0.8.0** (see
[SDK Changelog](/sdks/changelog) for wire deltas).

## Stable task facade (recommended)

`Client` is a hand-maintained facade for the common
...
`beeos.NewClient()` reads `BEEOS_API_KEY` automatically and uses
`https://openapi.beeos.ai`. Set `BEEOS_API_URL` for another environment, or
pass one `beeos.ClientOptions` value to override either setting explicitly.
