...
|---|---|
| `0.6.0` | [§ 2 — 0.6.0 → 0.6.1](#2-060--061) |
| `0.5.0` | [§ 3 — 0.5.0 → 0.6.0](#3-050--060), then § 2 |
| `0.4.x` | [§ 4 — 0.4.x → 0.5.0](#4-04x--050), then §§ 3–2 |
| `0.3.x` and earlier (pre-webhook-signing) | [§ 1 — 0.3 → 0.4](#1-03--04), then §§ 4–2 |
| Hand-rolled `fetch`/`net/http` directly against `openapi.beeos.ai` | [§ 5 — hand-rolled HTTP → SDK](#5-hand-rolled-http--sdk) |
| Migrating off `0.2.x` (pre-BFF decoupling) | [§ 6 — 0.2.x → 0.4.x](#6-02x--04x) |
...
`TaskStatus` success now matches the server's canonical wire value:

```text
succeeded -> completed
```

Update polling and terminal-state comparisons to use `completed`. This change
only applies to Platform Task status. Runtime Operation outcomes and webhook
delivery status have separate enums and continue to use `succeeded`.

---

## 3. `0.5.0` → `0.6.0`

This release has no wire-level migration. Existing generated-client calls
continue to work. You can adopt the new task facade incrementally:

```ts
import { BeeOSClient } from "@beeos-ai/sdk";

const client = new BeeOSClient({ apiKey: process.env.BEEOS_API_KEY! });
const agents = await client.listAgents();
```

For TypeScript consumers, Node.js 20 or newer is now required. Both ESM and
CommonJS remain supported through explicit package exports. Go callers can
adopt `NewClient(ClientOptions{...})` while retaining direct access to the
generated client through `Client.API`.

---

## 4. `0.4.x` → `0.5.0`

The entries below shipped together in `0.5.0`. None are breaking.

### 4.1 Webhook HMAC signing — opt-in but recommended
...
### 4.2 Webhook delivery audit log + manual replay
...
### 4.3 `GET /api/v1/tasks` — cross-agent inbox
...
### 4.4 `attachments[].file_id` on invoke / tasks / conversations
...
### 4.5 `PATCH /api/v1/agents/{agentId}`
...
### 4.6 Wire-error envelope tightened
...
### 4.7 Scope vocabulary removed (v1.1.0)
...
### 4.8 `timeout_ms` clamp on `invoke`
