...
### 2.7 New error code — `insufficient_scope`

If you mint an `oag_` User API Key with read-only scopes and
try a write operation, you now get `403 insufficient_scope`
(previously the call silently succeeded if the underlying user
was the owner). This is a **wire** change at the boundary
between "your code worked by accident" and "your code worked
because the auth model said so".

To fix: re-mint the key with the right scopes (see
[Auth & API Keys](/authentication#scopes)).
...
- [`docs/guides/auth-api-keys.md`](/authentication) — credential
  matrix for the `insufficient_scope` change.
