...
### 3.1 Webhook HMAC signing — opt-in but recommended
...
### 3.2 Webhook delivery audit log + manual replay
...
### 3.3 `GET /api/v1/tasks` — cross-agent inbox
...
### 3.4 `attachments[].file_id` on invoke / tasks / conversations
...
### 3.5 `PATCH /api/v1/agents/{agentId}`
...
### 3.6 Wire-error envelope tightened
...
### 3.7 Scope vocabulary removed (v1.1.0)
...
### 3.8 `timeout_ms` clamp on `invoke`
