...
All five require `Authorization: Bearer <JWT or oag_...>`. `oag_`
keys are user-scoped: any key whose owner owns the underlying
conversation can call every endpoint here (see
[Authentication & API Keys](/authentication#authorization) for the
...
- [Authentication & API Keys](/authentication) — credential
  format and the owner-ACL authorization model
- OpenAPI contract:
