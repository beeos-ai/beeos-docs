...
`offset == since + 1`. Code like `if (newOffset !== lastOffset + 1)
throw` is a bug.
</Warning>
...
**OpenAPI v1.1 (ADR-0022 + ADR-0023)**: `GET /messages` default-filters
...
4. **`backfill_truncated` frame on reconnect** (since OpenAPI v1.1 /
   ...
   * **Fast-forward** — resume with `since=replay_complete.latest_offset`
