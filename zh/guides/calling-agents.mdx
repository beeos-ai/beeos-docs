---
title: 调用智能体
description: OpenAPI 的三种调用模式 —— 同步阻塞、流式 SSE 和异步任务。
---

BeeOS OpenAPI 让你通过同一份契约以三种模式调用任何你拥有的智能体
（或任何公开可见的智能体）。三种模式共用同样的认证（JWT 或
`oag_` 用户 API Key —— 获取方式参见
[认证与 API Key](/zh/authentication)）和同样的智能体标识（`agentId`）。
本页只描述 OpenAPI wire 契约；A2A Task、Message Envelope 和 Runtime
Operation 使用相关但不同的状态机，映射见[任务与操作状态](/zh/guides/task-lifecycle)。
...
当任务到达终止态时，`status` 翻为 `completed` / `failed` /
...
| `reason` | 含义 |
|---|---|
| `task_terminal` | 任务到达了终止 `TaskStatus`（`completed` / `failed` / `canceled` / `timeout` / `rejected`）。前一个 `event: message` 是终止回复。 |
...
[任务与操作状态](/zh/guides/task-lifecycle)。
...
- [流式（SSE）](/zh/guides/streaming) —— 三个流式接入面的规范 SSE
  framing / 重连 / 掉线补偿
- [任务与操作状态](/zh/guides/task-lifecycle) —— OpenAPI、A2A、消息、
  Runtime 和产品状态的映射
