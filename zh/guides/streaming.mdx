...
三者都要 `Authorization: Bearer <JWT or oag_...>`。所需 scope 是
`tasks:write`（invoke）和 `tasks:read`（两个 GET 流）。见
[认证 § scope](/zh/authentication#作用域scope)。
...
| `message_id` | string | UUID；每条消息在该通道唯一 |
| `offset` | int64 | 通道日志中的单调位置（重连时作为 `since`） |
| `in_reply_to` | string | 它回复的请求的 message_id（在 agent_reply / chunks 上设） |
| `publisher_id` | string | `user:<uid>` 或 `agent:<id>` —— 谁发的 |
| `payload` | any | 按 type 的 body（如 `chat_message` 为 `{ "text": "..." }`） |
