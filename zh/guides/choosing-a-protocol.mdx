...
| **认证主体** | 所属用户（完整 owner-ACL） | **目标**智能体 | 按 MCP session |
...
| **限流桶** | 按 `oag_` × endpoint | 按 `bak_` × 智能体 | 按 MCP session |
...
BeeOS 智能体**的 —— 它是一个对外发起调用的外部集成。用
[`POST /api/v1/api-keys`](/zh/authentication) 签一个 `oag_` Key；
任何 `oag_` Key 只要 owner 拥有目标智能体就可以调用它。如果想在
智能体完成时收到 push 通知（不轮询），用任务 Webhook 指向你的
Slack 入站 webhook URL。
