...
| `agent-gw.beeos.ai` | 智能体 pod 的入站（Ed25519 签名） | Ed25519 签名 | **仅智能体进程**，从不是外部集成方 |

前三个面向调用方；第四个是**智能体 pod** 回家用的边界。如果你在智能体
进程之外写集成代码，你永远不会直接跟 `agent-gw.beeos.ai` 对话 —— 你的代码
...
| **状态术语** | OpenAPI 状态字符串（`completed` / `failed` / `canceled` / `timeout`）从 A2A `TaskState` 枚举用的同一个 `msgderive` resolver 派生。 |
