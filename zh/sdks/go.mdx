...
export BEEOS_API_KEY="oag_your_key"
```

当前发布版本：**v0.8.0**（wire delta 见 [SDK 变更日志](/zh/sdks/changelog)）。

## 稳定任务 facade（推荐）

`Client` 是覆盖常用 Agent 任务生命周期的手工维护 facade；
...
`beeos.NewClient()` 会自动读取 `BEEOS_API_KEY`，并默认连接
`https://openapi.beeos.ai`。如需切换环境，可设置 `BEEOS_API_URL`；也可以传入一个
`beeos.ClientOptions` 显式覆盖配置。

facade 同时提供 `CreateTask`、`GetTask`、`CancelTask` 和
`TaskEvents`。完整生成客户端仍可通过 `client.API` 或
`NewAPIClient` 使用。
