...
当前发布版本：**0.8.0**（wire delta 见 [SDK 变更日志](/zh/sdks/changelog)）。

## 稳定任务 facade（推荐）

`BeeOSClient` 是覆盖常用 Agent 任务生命周期的手工维护 facade；
...
`new BeeOSClient()` 会自动读取 `BEEOS_API_KEY`，并默认连接
`https://openapi.beeos.ai`。如需切换环境，可设置 `BEEOS_API_URL`；也可以显式传入
`{ apiKey, baseUrl }` 覆盖默认值。
