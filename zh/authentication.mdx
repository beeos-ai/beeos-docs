...
有效（通常 1 小时，SDK / Web 客户端会自动续期）。**携带用户的完整作用域** ——
凡是用户在 Web UI 能做的操作，用 JWT 都能做。
...
OpenAPI Gateway 按路由强制 scope。每个请求至少需要**一个**该路由列出的
scope；一个都不匹配则返回 `403 insufficient_scope`，
`error.metadata.required_scope` 字段里以逗号分隔回显可接受的 scope
集合（任一即可）。
...
| `tasks:read` | `GET /agents/{agentId}/tasks*`、`GET /agents/{agentId}/conversations*`（含历史、SSE 流） |
| `tasks:write` | `POST /agents/{agentId}/invoke`、所有 `POST /agents/{agentId}/tasks*`（创建、取消、续传）、所有 `POST /agents/{agentId}/conversations*`（创建、发送）、`DELETE /conversations/{convId}`、Webhook CRUD（`POST` / `DELETE /tasks/{taskId}/webhooks*`） |
...
| `instances:write` | 所有 `POST /instances*` 和 `DELETE /instances/{id}` |

<Note>
**JWT 调用方绕过 scope 检查。** 用户在 Dashboard 里已经拥有全部接入面；
scope 只对 `oag_` Key 生效。
</Note>

<Note>
签发 Key 时如果忘了设 scope，请求会被拒绝 —— 没有"全 scope 默认"。
请用满足任务的**最小** scope 集合。
</Note>
...
- **把 `oag_` Key 嵌进发布的移动端 / 桌面端二进制** —— 用户的 Key
  ...
  服务端 `oag_` 完成后端工作。
- **忘了设 scope** —— scope 强制是实时的；缺所需 scope 的 Key 返回
  `403 insufficient_scope`，`error.metadata.required_scope` 里回显
  可接受的 scope 集合。按[上文表格](#scope-scope)选最窄的 scope。
