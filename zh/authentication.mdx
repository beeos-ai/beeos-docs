...
|---|---|---|---|
| **用户 JWT** | `Authorization: Bearer <jwt>` | `api.beeos.ai`（Web / 移动端 / 桌面）**和** `openapi.beeos.ai`（SDK） | 用户登录（Gateway `POST /api/v1/auth/login`） |
...
由主 Gateway（`api.beeos.ai`，本地 `:9080`）在登录时签发。短期
