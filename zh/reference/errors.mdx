...
| `insufficient_scope` | 403 | JSON | API key 缺该路由所需 scope —— 见 [scope 表](/zh/authentication#作用域scope)。`error.metadata.required_scope` 以逗号分隔回显可接受的 scope 集合（任一即可）。JWT 调用方永远不会撞这个 | 签发带正确 scope 的新 key；轮换调用方 |
