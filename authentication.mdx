...
|---|---|---|---|
| **User JWT** | `Authorization: Bearer <jwt>` | `api.beeos.ai` (web / mobile / desktop) **and** `openapi.beeos.ai` (SDK) | User login (Gateway `POST /api/v1/auth/login`) |
...
Issued by the main Gateway (`api.beeos.ai`, local `:9080`) on
