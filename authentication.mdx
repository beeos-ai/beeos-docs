...
SDK / web client). **Carries the user's full scope** — any operation
the user can do from the web UI works with their JWT.
...
OpenAPI Gateway enforces scopes per route. Each request needs **at
least one** of the scopes listed for that route; a request that
matches none returns `403 insufficient_scope` with the acceptable
scope set echoed in `error.metadata.required_scope` (comma-separated;
any one is sufficient).

| Scope | Routes |
|---|---|
| `agents:read` | `GET /agents`, `GET /agents/{agentId}` |
| `agents:write` | `PATCH /agents/{agentId}` |
| `tasks:read` | `GET /agents/{agentId}/tasks*`, `GET /agents/{agentId}/conversations*` (incl. history, SSE stream) |
| `tasks:write` | `POST /agents/{agentId}/invoke`, all `POST /agents/{agentId}/tasks*` (create, cancel, continue), all `POST /agents/{agentId}/conversations*` (create, send), `DELETE /conversations/{convId}`, webhook CRUD (`POST` / `DELETE /tasks/{taskId}/webhooks*`) |
| `files:read` | `GET /files/{id}` |
| `files:write` | `POST /files/presign-upload` |
| `instances:read` | `GET /providers`, `GET /deploy/regions`, `GET /deploy/models`, `GET /instances*` |
| `instances:write` | All `POST /instances*` and `DELETE /instances/{id}` |

<Note>
**JWT callers bypass the scope check.** A user in the dashboard
already has the full surface; scopes only apply to `oag_` keys.
</Note>
>
<Note>
If you forget to set scopes when minting a key the request is
denied — there is no "all scopes" implicit default. Pick the
...
- **Forgetting to set scopes** — scope enforcement is live; a key
  missing the required scope returns `403 insufficient_scope` with
  the acceptable scope set echoed in `error.metadata.required_scope`.
  Pick the narrowest scope set per [the table above](#scopes).
