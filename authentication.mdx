...
| **Agent API Key** | `bak_` | Single agent (bound to `agentId`) | `AgentIdentityService.IssueAgentAPIKey` or Dashboard |
...
| Gateway | URL | Accepts |
|---------|-----|---------|
| **OpenAPI Gateway** | `openapi.beeos.ai` | `oag_` User API Key, JWT |
| **A2A Gateway** | `a2a.beeos.ai` | `bak_` Agent API Key, JWT, `oag_` |
| **MCP Gateway** | `mcp.beeos.ai` | `bak_` Agent API Key, `oag_` User API Key, OAuth 2.1 Bearer |
...
sequenceDiagram
    ...
    Web->>Auth: user logs in
    Auth-->>Web: login ticket
    Web->>GW: redirect back with ticket
...
| A2A Gateway | Per-agent rate limiting via Redis |
