...
| `agent-gw.beeos.ai` | Inbound for agent pods (Ed25519-signed) | Ed25519 signatures | **Agent processes only**, never external integrators |
...
| **Status terminology** | OpenAPI status strings (`completed` / `failed` / `canceled` / `timeout`) are derived from the same `msgderive` resolver A2A uses for its `TaskState` enum. |
